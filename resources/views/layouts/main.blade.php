    ...
    >
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
            ...
            @if (Auth::user()?->canAccessMenu('laporan'))
                <x-sidebar-link href="{{ route('laporan.bulanan') }}" wire:navigate :active="request()->routeIs('laporan*')">
                    <x-slot name="icon">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            {{-- Master Data --}}
            @if (Auth::user()?->canAccessMenu('pengguna') || Auth::user()?->canAccessMenu('wilayah-lingkungan') || Auth::user()?->canAccessMenu('role-permission'))
                <div class="pt-4 pb-1">
